...
#  index_staff_authorization_codes_on_varnishable_at  (varnishable_at)
#
require "test_helper"
...
class StaffAuthorizationCodeTest < ActiveSupport::TestCase
  def setup
    ...
    @code = StaffAuthorizationCode.issue!(
      subject: @staff,
  ...
  test "inherits from OperatorRecord" do
    assert_operator StaffAuthorizationCode, :<, OperatorRecord
  end

  test "belongs to staff" do
    association = StaffAuthorizationCode.reflect_on_association(:staff)
  ...
  test "includes OidcAuthorizationCode concern" do
    assert_includes StaffAuthorizationCode.ancestors, OidcAuthorizationCode
  end

  test "subject_association_name returns :staff" do
    assert_equal :staff, StaffAuthorizationCode.subject_association_name
  end
  ...
  test "code is unique" do
    duplicate = StaffAuthorizationCode.new(
      staff: @staff,
      code: @code.code,
  ...
  test "subject returns the associated staff" do
    assert_equal @staff, @code.subject
  end

  test "subject_id returns the staff_id" do
    assert_equal @staff.id, @code.subject_id
  end

  test "valid scope returns non-expired, non-consumed, non-revoked codes" do
    expired = StaffAuthorizationCode.issue!(
      subject: @staff,
      client_id: "test",
      redirect_uri: "https://example.com",
      code_challenge: "challenge",
    ...
    consumed = StaffAuthorizationCode.issue!(
      subject: @staff,
      client_id: "test",
      redirect_uri: "https://example.com",
      code_challenge: "challenge",
    ...
    revoked = StaffAuthorizationCode.issue!(
      subject: @staff,
      client_id: "test",
      redirect_uri: "https://example.com",
      code_challenge: "challenge",
    ...
    valid_codes = StaffAuthorizationCode.valid.pluck(:id)
  ...
  test "staff association loads staff correctly" do
    assert_equal @staff, @code.staff
    assert_equal @staff.id, @code.staff.id
  end
  ...
  test "association deletion: destroys when staff is destroyed" do
    @code.reload
    @staff.destroy
  ...
  test "index on varnishable_at exists" do
    indexes = StaffAuthorizationCode.connection.indexes("staff_authorization_codes")
  ...
  test "index on staff_id exists" do
    indexes = StaffAuthorizationCode.connection.indexes("staff_authorization_codes")
    staff_id_index = indexes.find { |i| i.columns.include?("staff_id") }

    assert_not_nil staff_id_index, "Expected index on staff_id to exist"
  end
