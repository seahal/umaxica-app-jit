...
class SocialAuthAutoLinkTest < ActionDispatch::IntegrationTest
  ...
  test "logged-in user: Apple callback automatically links UserSocialApple" do
    ...
    get identity.sign_app_social_start_url(provider: "apple", intent: "link", ri: "jp"),
        headers: @callback_headers.merge(as_user_headers(user, host: @host))
    get Rails.application.routes.url_helpers.sign_identity.app_auth_callback_url(provider: "apple", ri: "jp"),
        headers: @callback_headers.merge(as_user_headers(user, host: @host))

    # Should redirect to success path (configuration page)
    assert_response :redirect
    assert_redirected_to identity.sign_app_configuration_url(ri: "jp")
    follow_redirect!
  ...
  test "logged-in user: Google callback automatically links UserSocialGoogle" do
    ...
    get identity.sign_app_social_start_url(provider: "google_app", intent: "link", ri: "jp"),
        headers: @callback_headers.merge(as_user_headers(user, host: @host))
    get Rails.application.routes.url_helpers.sign_identity.app_auth_callback_url(provider: "google_app", ri: "jp"),
        headers: @callback_headers.merge(as_user_headers(user, host: @host))
  ...
  test "idempotent: calling Apple callback twice does not create duplicate" do
    ...
    get identity.sign_app_social_start_url(provider: "apple", intent: "link", ri: "jp"),
        headers: @callback_headers.merge(as_user_headers(user, host: @host))
    get identity.sign_app_auth_callback_url(provider: "apple", ri: "jp"),
        headers: @callback_headers.merge(as_user_headers(user, host: @host))
  ...
  test "not logged in: Apple callback creates new user (login flow, not link)" do
    ...
    get identity.sign_app_social_start_url(provider: "apple", intent: "login", ri: "jp"),
