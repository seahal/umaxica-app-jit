...
class OrgStepUpVerificationEnforcerTest < ActionDispatch::IntegrationTest
  ...
  test "GET protected endpoint redirects to setup when configured methods are zero" do
    StepUp::ConfiguredMethods.stub(:call, []) do
      StepUp::AvailableMethods.stub(:call, []) do
        get identity.sign_org_configuration_withdrawal_url(ri: "jp"), headers: @headers
      end
    end
  ...
  test "POST protected endpoint returns 401 plain when step-up is missing and usable methods exist" do
    ...
    post identity.options_sign_org_configuration_passkeys_url(ri: "jp"), headers: @headers
  ...
  test "successful verification enables protected POST and records audit" do
    ...
    return_to = Base64.urlsafe_encode64(identity.sign_org_configuration_passkeys_path(ri: "jp"))
    trusted_origins = [
      "http://#{@host}",
      "https://#{@host}",
      "http://#{@host}:3000",
    ...
    Webauthn.stub(:trusted_origins, trusted_origins) do
      get identity.sign_org_verification_url(scope: "configuration_passkey", return_to: return_to, ri: "jp"),
          headers: @headers

      get identity.new_sign_org_verification_passkey_url(ri: "jp"), headers: @headers
      ...
      WebAuthn::Credential.stub(:from_get, mock_credential) do
        post identity.sign_org_verification_passkey_url(ri: "jp"), params: {
          verification: {
    ...
    assert_response :redirect
    assert_redirected_to identity.sign_org_configuration_passkeys_url(ri: "jp")
