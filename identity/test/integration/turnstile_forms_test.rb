...
class TurnstileFormsTest < ActionDispatch::IntegrationTest
  def setup
    ...
    @identity.turnstile_form_paths = [
      { name: "Sign::App registration emails", env_key: "IDENTITY_SIGN_APP_URL", path: "/up/emails/new" },
      { name: "Sign::App authentication email", env_key: "IDENTITY_SIGN_APP_URL", path: "/in/email/new" },
      # { name: "Sign::Org registration emails", env_key: "IDENTITY_SIGN_ORG_URL", path: "/registration/emails/new" },
      # { name: "Sign::Org registration passkeys", env_key: "IDENTITY_SIGN_ORG_URL", path: "/registration/passkeys/new" },
      {
        name: "JitBase::App contacts",
        env_key: "FOUNDATION_BASE_APP_URL",
        path: "/contacts/new",
        headers: base_app_contact_headers,
      },
      { name: "JitBase::Com contacts", env_key: "FOUNDATION_BASE_COM_URL", path: "/contacts/new" },
    ]
  end
