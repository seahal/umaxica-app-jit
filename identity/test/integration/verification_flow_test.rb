...
class VerificationFlowTest < ActionDispatch::IntegrationTest
  ...
  test "high-risk operation redirects to verification when step-up not satisfied" do
    ...
    get identity.sign_app_configuration_emails_url(ri: "jp"), headers: @headers
  ...
  test "high-risk operation redirects to verification when step-up not satisfied (HEAD)" do
    ...
    # Try to access email configuration (requires step-up)
    head identity.sign_app_configuration_emails_url(ri: "jp"), headers: @headers
  ...
  test "successful passkey verification redirects to return_to" do
    return_to = Base64.urlsafe_encode64(identity.sign_app_configuration_emails_path(ri: "jp"))
    ...
    Webauthn.stub(:trusted_origins, trusted_origins) do
      get identity.sign_app_verification_url(scope: "configuration_email", return_to: return_to, ri: "jp"),
          headers: @headers

      get identity.new_sign_app_verification_passkey_url(ri: "jp"), headers: @headers
      ...
      WebAuthn::Credential.stub(:from_get, mock_credential) do
        post identity.sign_app_verification_passkey_url(ri: "jp"), params: {
          verification: {
    ...
    assert_response :redirect
    assert_redirected_to identity.sign_app_configuration_emails_url(ri: "jp")
  end
end
