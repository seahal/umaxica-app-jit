...
class Sign::App::Configuration::PasskeysControllerTest < ActionDispatch::IntegrationTest
  ...
  setup do
    ...
    @headers = as_user_headers(@user, host: ENV.fetch("IDENTITY_SIGN_APP_URL", "sign.app.localhost")).merge(
    ...
    allowed_origins = [
      ...
      "http://#{ENV.fetch("IDENTITY_SIGN_APP_URL", "sign.umaxica.app")}",
      "https://#{ENV.fetch("IDENTITY_SIGN_APP_URL", "sign.umaxica.app")}",
    ].uniq
  ...
  test "options redirects when not logged in" do
    post identity.options_sign_app_configuration_passkeys_path(ri: "jp")
  ...
  test "options returns challenge and options" do
    post identity.options_sign_app_configuration_passkeys_path(ri: "jp"), headers: @headers
  ...
  test "verification fails with unknown challenge" do
    ...
    post identity.verification_sign_app_configuration_passkeys_path(ri: "jp"), params: params, headers: @headers
  ...
  test "verification creates passkey on success" do
    ...
    WebAuthn::Credential.stub(:from_create, mock_credential) do
      ...
      assert_predicate response.parsed_body["identity.redirect_url"], :present?
    end
  end
  ...
  test "create returns unprocessable entity plain message when user has no verified recovery identity" do
    unverified_user = User.create!(status_id: UserStatus::NOTHING, public_id: SecureRandom.hex(10))
    headers = as_user_headers(unverified_user, host: ENV.fetch("IDENTITY_SIGN_APP_URL", "sign.app.localhost"))
  ...
  test "blocks deleting last passkey when only secret remains" do
    ...
    user.user_emails.each { |email| email.update!(user_email_status_id: UserEmailStatus::UNVERIFIED) }
