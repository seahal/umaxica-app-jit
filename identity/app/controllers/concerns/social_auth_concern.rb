...
module SocialAuthConcern
  ...
  def social_auth_failure_redirect_path
    respond_to?(:new_sign_app_in_path) ? identity.new_sign_app_in_path : "/"
  end

  # Override this method to customize the success redirect path
  def social_auth_success_redirect_path
    respond_to?(:sign_app_root_path) ? identity.sign_app_root_path : "/"
  end
  ...
  def social_auth_failure_redirect_path_for_intent(intent:, provider:)
    ...
    if provider.to_s == "apple"
      return identity.sign_app_configuration_apple_path if respond_to?(:sign_app_configuration_apple_path, true)
    ...
    return identity.sign_app_configuration_path if respond_to?(:sign_app_configuration_path, true)
