  ...
  module App
    module Configuration
      ...
      class PasskeysController < ApplicationController
        ...
        before_action :set_passkey_or_404, only: []
        ...
        def update
          ...
          if @passkey.update(update_params)
            respond_to do |format|
              format.html do
                redirect_to(
                  identity.sign_app_configuration_passkey_path(@passkey),
        ...
        def destroy
          ...
          unless AuthMethodGuard.can_remove_passkey?(current_user, @passkey)
            respond_to do |format|
              format.html do
                redirect_to(
                  identity.sign_app_configuration_passkeys_path,
                  status: :see_other,
        ...
        def persist_passkey!(passkey)
          authorize(passkey, :create?)
          passkey.save!
          audit_passkey_registered!(passkey)
        end

        def audit_passkey_registered!(passkey)
          ActivityRecord.connected_to(role: :writing) do
            UserActivityEvent.find_or_create_by!(id: UserActivityEvent::PASSKEY_REGISTERED)
          ...
          UserActivity.create!(
            ...
            event_id: UserActivityEvent::PASSKEY_REGISTERED,
            subject_id: passkey.id.to_s,
            subject_type: "UserPasskey",
            ip_address: request.remote_ip,
            occurred_at: Time.current,
          )
        rescue ActiveRecord::RecordInvalid => e
          Rails.event.error(
            "sign.passkey.registration.audit_failed",
        ...
        def audit_passkey_removed!(passkey)
          ActivityRecord.connected_to(role: :writing) do
            UserActivityEvent.find_or_create_by!(id: UserActivityEvent::PASSKEY_REMOVED)
            UserActivityLevel.find_or_create_by!(id: UserActivityLevel::NOTHING)
          end
          ...
          UserActivity.create!(
            actor_type: "User",
            actor_id: current_user.id,
            event_id: UserActivityEvent::PASSKEY_REMOVED,
            subject_id: passkey.id.to_s,
            subject_type: "UserPasskey",
            ip_address: request.remote_ip,
            occurred_at: Time.current,
          ...
          Rails.event.error(
            "sign.passkey.removal.audit_failed",
            user_id: current_user.id,
            passkey_id: passkey.id,
            errors: e.record.errors.full_messages,
          )
        ...
        def render_verification_success
          redirect_url =
            if respond_to?(:sign_app_configuration_emergency_key_path, true)
              identity.sign_app_configuration_emergency_key_path(ri: params[:ri])
            else
              identity.sign_app_configuration_passkeys_path(ri: params[:ri])
            end
