  ...
  module App
    module Configuration
      class SecretsController < ApplicationController
        ...
        before_action :set_secret_or_404, only: [:regenerate]
        ...
        def create
          ...
          redirect_to(identity.sign_app_configuration_secrets_path)
        ...
        def destroy
          if AuthMethodGuard.last_method?(current_user, excluding: @secret)
            ...
            return redirect_to(identity.sign_app_configuration_secrets_path)
          end

          UserSecrets::Destroy.call(actor: current_user, secret: @secret)
          flash[:notice] = t(".destroyed")
          redirect_to(identity.sign_app_configuration_secrets_path, status: :see_other)
        end

        # Reserved for future secret rotation support.
        def regenerate
          redirect_to(
            identity.sign_app_configuration_secret_path(@secret.public_id),
