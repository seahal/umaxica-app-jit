...
module Jit
  module Zenith
    ...
    class Jit::Zenith::Acme::App::Auth::CallbacksControllerTest < ActionDispatch::IntegrationTest
      setup do
      ...
      test "returns client_id as acme_app" do
        controller = Jit::Zenith::Acme::App::Auth::CallbacksController.new
      ...
      test "callback route exists" do
        assert_routing(
          ...
          { host: @host, controller: "jit/zenith/acme/app/auth/callbacks", action: "show" },
        )
      end
      ...
      test "oidc_callback_url omits default https port" do
        harness = build_oidc_callback_harness(host: @host, scheme: "https", port: 443, client_id: "acme_app")
      ...
      test "oidc_callback_url omits default http port" do
        harness = build_oidc_callback_harness(host: @host, scheme: "http", port: 80, client_id: "acme_app")
      ...
      test "GET show with valid state and code exchanges token and redirects to return_to" do
        ...
        freeze_time do
          ...
          expected_args = {
            ...
            client_id: "acme_app",
            client_secret: Oidc::ClientRegistry.find("acme_app")&.client_secret,
