...
module Jit
  module Zenith
    ...
    class Jit::Zenith::Acme::App::Web::V0::CookieControllerTest < ActionDispatch::IntegrationTest
      ...
      test "GET show without access jwt returns consented false" do
        ...
        get zenith.acme_app_web_v0_cookie_path, as: :json
      ...
      test "PATCH update returns 200 and sets preference_consented cookie with app domain" do
        ...
        travel_to(expires_at - Preference::Base::REFRESH_TOKEN_TTL) do
          ...
          with_cookie_domain_credentials(COOKIE_DOMAIN_APP: ".app.localhost") do
            with_preference_jwt_keys(host: @host) do
              patch zenith.acme_app_web_v0_cookie_path, as: :json
            end
          end
        end
      ...
      test "PATCH update with consented true updates preference cookie and issues access token" do
        ...
        with_preference_jwt_keys(host: @host) do
          patch zenith.acme_app_web_v0_cookie_path, params: { consented: true }, as: :json
        end
      ...
      test "PATCH update rolls back consent when access token issue fails" do
        preference = AppPreference.create!(status_id: AppPreferenceStatus::NOTHING)
        ...
        # Stub Preference::Token.encode to simulate failure during access token issuance
        with_preference_jwt_keys(host: @host) do
          Preference::Token.stub(:encode, ->(*_args) { raise NoMethodError, "token encoding failed" }) do
