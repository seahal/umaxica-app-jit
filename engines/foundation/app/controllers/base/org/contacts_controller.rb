...
module Jit
  module Foundation
    module Base
      module Org
        class ContactsController < Jit::Foundation::Base::Org::ApplicationController
          include ::RateLimit
          ...
          def create
            ...
            ActiveRecord::Base.transaction do
              @contact.status_id = OrgContactStatus::COMPLETED
              @contact.save!
            ...
            redirect_to(
              foundation.base_org_contact_url(@contact, **base_staff_redirect_options),
          ...
          def turnstile_stealth_valid?
            if Jit::Security::TurnstileConfig.stealth_secret_key.blank?
              Rails.event.notify("contact.turnstile.stealth_skipped", controller: "base/org/contacts")
              return true
            end
          ...
          def write_behavior_event(contact)
            Rails.event.record(
              ...
              staff_id: contact_actor_context.actor_id,
          ...
          def contact_actor_context
            @contact_actor_context ||= Contact::ActorContext.new(actor: current_staff)
          end

          def base_staff_redirect_options
            {
          ...
          def core_staff_host
            env_url = ENV["FOUNDATION_BASE_ORG_URL"].presence
