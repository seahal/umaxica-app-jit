        ...
        class HealthsControllerTest < ActionDispatch::IntegrationTest
          test "should render healthy status" do
            with_stubbed_health_status([200, "OK"]) do
              get foundation.base_org_health_url
          ...
          test "should render unhealthy status with dependency details" do
            with_stubbed_health_status([503, "UNHEALTHY", ["Database ActivityRecord(writing) unavailable"]]) do
              get foundation.base_org_health_url(format: :html)
          ...
          def with_stubbed_health_status(result)
            Jit::Foundation::Base::Org::HealthsController.send(:define_method, :get_status) { result }
            yield
          ensure
            if Jit::Foundation::Base::Org::HealthsController.private_method_defined?(:get_status) ||
                Jit::Foundation::Base::Org::HealthsController.method_defined?(:get_status, false)
              Jit::Foundation::Base::Org::HealthsController.send(:remove_method, :get_status)
            end
          end
        end
      end
