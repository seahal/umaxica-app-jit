...
class SocialLoginRobustnessTest < ActionDispatch::IntegrationTest
  ...
  test "social login with invalid oauth state is rejected gracefully" do
    ...
    get identity.sign_app_auth_callback_url(provider: "google_app", ri: "jp", state: "invalid_state"),
        headers: { "Host" => @host }
  ...
  test "social login callback error handling is robust" do
    ...
    begin
      get(
        identity.sign_app_auth_callback_url(provider: "google_app", ri: "jp"),
        headers: { "Host" => @host },
      )
    ensure
  ...
  test "reauth updates last_reauth_at timestamp" do
    ...
    # Start reauth
    get identity.sign_app_social_start_url(provider: "google_app", intent: "reauth", ri: "jp"),
        headers: as_user_headers(user, host: @host)

    assert_response :redirect

    # Now do the callback with reauth
    post identity.sign_app_auth_callback_url(provider: "google_app", ri: "jp"),
  ...
  test "social login does not require additional MFA during callback" do
    ...
    # Start social login
    get identity.sign_app_social_start_url(provider: "google_app", intent: "login", ri: "jp"),
        headers: { "Host" => @host }
    ...
    # should not fail
    get identity.sign_app_auth_callback_url(provider: "google_app", ri: "jp"),
