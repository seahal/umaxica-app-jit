...
class AppStepUpVerificationEnforcerTest < ActionDispatch::IntegrationTest
  ...
  test "POST protected endpoint returns 401 plain when step-up is missing and usable methods exist" do
    ...
    post identity.sign_app_configuration_withdrawal_url(ri: "jp"), headers: @headers
  ...
  test "successful verification enables protected POST and records audit" do
    ...
    return_to = Base64.urlsafe_encode64(identity.sign_app_configuration_withdrawal_path(ri: "jp"))

    get identity.sign_app_verification_url(scope: "withdrawal", return_to: return_to, ri: "jp"), headers: @headers

    assert_response :success

    code = ROTP::TOTP.new(private_key).at(Time.current.to_i)
    post identity.sign_app_verification_totp_url(ri: "jp"),
         params: { verification: { code: code } },
         headers: @headers

    assert_response :redirect
    assert_redirected_to identity.sign_app_configuration_withdrawal_url(ri: "jp")
