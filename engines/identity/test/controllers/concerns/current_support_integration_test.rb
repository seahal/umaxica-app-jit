...
class CurrentSupportIntegrationTest < ActionDispatch::IntegrationTest
  ...
  test "acme request falls back to JWT preference payload and resets Current after response" do
    user = User.create!(public_id: "u_#{SecureRandom.hex(8)}", status_id: UserStatus::NOTHING)

    with_test_routes do
      get "/acme/current_support",
          headers: {
            "Host" => "app.localhost",
            "X-TEST-ACTOR-CLASS" => "User",
            "X-TEST-ACTOR-ID" => user.id.to_s,
  ...
  def with_test_routes
    with_routing do |set|
      set.draw do
        ...
        get("/acme/current_support", to: "acme/app/current_support_dummy#show")
