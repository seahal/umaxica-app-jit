    ...
    class Sign::Org::In::PasskeysControllerTest < ActionDispatch::IntegrationTest
      ...
      setup do
        ...
        Webauthn.define_singleton_method(:trusted_origins) do
          ["http://sign.app.localhost", "https://sign.app.localhost", "http://#{host}", "https://#{host}"]
        end
      ...
      test "rejects logged in staff on passkey login page" do
        staff = staffs(:one)
      ...
      test "options returns error if identifier blank" do
        post identity.options_sign_org_in_passkeys_url(ri: "jp"), params: { identifier: "" }
      ...
      test "options returns error if identifier not found" do
        post identity.options_sign_org_in_passkeys_url(ri: "jp"), params: { identifier: "unknown@example.com" }

        assert_response :unprocessable_content
        assert_includes response.body, I18n.t("errors.webauthn.identifier_invalid")
      end

      test "options rejects logged in staff" do
        staff = staffs(:one)

        post identity.options_sign_org_in_passkeys_url(ri: "jp"),
             params: { identifier: staff.public_id },
             headers: { "Host" => @host, "X-TEST-CURRENT-STAFF" => staff.id }
      ...
      test "options returns error if staff has no passkeys" do
        ...
        post identity.options_sign_org_in_passkeys_url(ri: "jp"), params: { identifier: staff_no_passkey.public_id }
      ...
      test "options rejects email identifier" do
        post identity.options_sign_org_in_passkeys_url(ri: "jp"), params: { identifier: "staff_test@example.com" }
      ...
      test "options returns challenge and allowCredentials for lowercase staff public_id identifier" do
        post identity.options_sign_org_in_passkeys_url(ri: "jp"), params: { identifier: @staff.public_id.downcase }
      ...
      test "options returns challenge and allowCredentials for staff public_id identifier" do
        post identity.options_sign_org_in_passkeys_url(ri: "jp"), params: { identifier: @staff.public_id }
      ...
      test "verification returns error if challenge_id blank" do
        post identity.verification_sign_org_in_passkeys_url(ri: "jp"), params: { challenge_id: "" }
      ...
      test "verification returns error if challenge invalid" do
        post identity.verification_sign_org_in_passkeys_url(ri: "jp"), params: { challenge_id: "invalid_challenge" }
      ...
      test "verification logs staff in on success" do
        ...
        WebAuthn::Credential.stub(:from_get, mock_credential) do
          ...
          # Should log in
          post identity.verification_sign_org_in_passkeys_url(ri: "jp", rd: "/configuration/passkeys"), params: params
          ...
          assert_equal sign_org_root_path(ri: "jp"), json["identity.redirect_url"]
      ...
      test "verification returns unauthorized for credential mismatch" do
        ...
        WebAuthn::Credential.stub(:from_get, mock_credential) do
          post identity.verification_sign_org_in_passkeys_url(ri: "jp"), params: {
            challenge_id: challenge_id,
            credential: {
      ...
      test "verification returns 422 when staff becomes login forbidden before login" do
        post identity.options_sign_org_in_passkeys_url(ri: "jp"), params: { identifier: @staff.public_id }
        challenge_id = response.parsed_body["challenge_id"]
        ...
        @staff.update!(status_id: StaffStatus::RESERVED)
      ...
      test "verification for staff with mfa enabled succeeds (MFA not enforced for staff)" do
        ...
        WebAuthn::Credential.stub(:from_get, mock_credential) do
          ...
          post identity.verification_sign_org_in_passkeys_url(ri: "jp"), params: params
      ...
      test "verification returns session_restricted when one logical session has many rotated ancestors" do
        ...
        assert_equal sign_org_in_session_path(ri: "jp"), json["identity.redirect_url"]
      ...
      def create_rotated_active_staff_session(staff, rotations:)
        ...
        rotations.times do
          refresh = Jit::Identity::Sign::RefreshTokenService.call(refresh_token: refresh)[:refresh_token]
        end
      end
    end
  end
