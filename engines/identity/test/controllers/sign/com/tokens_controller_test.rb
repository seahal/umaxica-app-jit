  ...
  module Identity
    ...
    class Sign::Com::TokensControllerTest < ActionDispatch::IntegrationTest
      setup do
        ...
        @client_secret = "test_secret_for_core_com"
      end
      ...
      test "exchanges valid authorization code for tokens" do
        ...
        with_authenticated_client do
          post sign_com_token_url(host: @host, ri: "jp"), params: token_params(
      ...
      test "creates customer token record" do
        code_record = issue_code!
      ...
      test "rejects non-json browser-style requests" do
        code_record = issue_code!

        with_authenticated_client do
          post sign_com_token_url(host: @host, ri: "jp", format: :html), params: token_params(
            code: code_record.code,
          ), headers: browser_headers
        end
      ...
      def with_authenticated_client
        Oidc::ClientRegistry.stub(
          :authenticate, ->(cid, sec) {
                           cid == "core_com" && sec == @client_secret
                         },
        ) do
          yield
        end
