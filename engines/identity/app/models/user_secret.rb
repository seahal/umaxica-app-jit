...
class UserSecret < PrincipalRecord
  ...
  IDENTITY_SIGN_IN_ALLOWED_STATUS_IDS = [UserSecretStatus::ACTIVE].freeze
  IDENTITY_SIGN_IN_ALLOWED_KIND_IDS = UserSecretKind::ALLOWED_FOR_SECRET_SIGN_IN
  ...
  scope :allowed_for_secret_sign_in, lambda {
    where(
      user_identity_secret_status_id: IDENTITY_SIGN_IN_ALLOWED_STATUS_IDS,
      user_secret_kind_id: IDENTITY_SIGN_IN_ALLOWED_KIND_IDS,
    )
  }
  ...
  def sign_in_status_allowed?
    IDENTITY_SIGN_IN_ALLOWED_STATUS_IDS.include?(user_secret_status_id)
  end

  def sign_in_kind_allowed?
    IDENTITY_SIGN_IN_ALLOWED_KIND_IDS.include?(user_secret_kind_id)
  end
