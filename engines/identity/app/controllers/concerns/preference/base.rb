...
module Preference
  ...
  module JwtConfiguration
    ...
    # e.g., host "sign.umaxica.app" results in only audiences ending with ".app" or equal to an ".app" acme.
  ...
  module Base
    ...
    def cookie_banner_endpoint_url
      ...
      @cookie_banner_endpoint_url ||=
        begin
          endpoint_url = nil
          %i(
            zenith.acme_app_web_v0_cookie_url
            zenith.acme_com_web_v0_cookie_url
            zenith.acme_org_web_v0_cookie_url
          ).each do |helper_name|
    ...
    def cookie_banner_endpoint_available_for_request?
      expected_host =
        case ::Foundation::Surface.current(request)
        when :app then ENV["ZENITH_ACME_APP_URL"]
        when :com then ENV["ZENITH_ACME_COM_URL"]
        when :org then ENV["ZENITH_ACME_ORG_URL"]
        end
    ...
    def preference_dbsc_path
      case preference_class.name
      when "AppPreference"
        zenith.acme_app_edge_v0_dbsc_path
      when "OrgPreference"
        zenith.acme_org_edge_v0_dbsc_path
      when "ComPreference"
        zenith.acme_com_edge_v0_dbsc_path
      end
    end
    ...
    def access_token_cookie_name
      self.class.name.start_with?("Jit::Zenith::Acme::App::Preference") ? Authentication::Base::ACCESS_COOKIE_KEY : Preference::CookieName.access
    end
