...
module Jit
  module Identity
      ...
      module Com
        class ApplicationController < ::ApplicationController
          def self.local_prefixes
            ...
            app_prefix = controller_path.sub("/com/", "/app/").delete_prefix('jit/identity/')
          ...
          include Jit::Identity::Sign::Com::RouteAliasHelper
          include ::CurrentSupport
          include ::Finisher

          helper Jit::Identity::Sign::Com::ApplicationHelper
          helper_method :current_user if respond_to?(:helper_method)

          protect_from_forgery using: :header_or_legacy_token,
                               trusted_origins: ENV.fetch(
                                 "IDENTITY_SIGN_COM_TRUSTED_ORIGINS",
                                 "http://sign.com.localhost,https://sign.com.localhost",
                               )
                                 .split(",").map(&:strip),
                               with: :exception

          guest_only! # Public surface requires guest access by default
          ...
          def actor_verification_path(attrs)
            identity.sign_com_verification_path(attrs)
          end
          ...
          def verification_setup_redirect_path
            identity.new_sign_com_verification_setup_path(ri: params[:ri], rd: encoded_step_up_rd)
          end

          def after_login_path
            if current_customer&.respond_to?(:verified_telephone?) && !current_customer.verified_telephone?
              return identity.new_sign_com_configuration_telephones_registration_path(ri: params[:ri])
            end

            identity.sign_com_configuration_path
          ...
          def enforce_required_telephone_registration!
            ...
            redirect_to(
              identity.new_sign_com_configuration_telephones_registration_path(ri: params[:ri]),
    ...
    Jit::Identity::Sign::Com::ApplicationController.send(:public, :current_user)
  end
end
