...
module Jit::Identity
    class Sign::Org::In::SessionsController < Jit::Identity::Sign::Org::ApplicationController
      ...
      def destroy
        ...
        if ref.present?
          ...
          log_out
          redirect_to(
            identity.new_sign_org_in_path, notice: I18n.t(
      ...
      def redirect_to_return_path(notice:)
        ...
        if return_path.present?
          flash[:notice] = notice
          jump_to_generated_url(return_path, fallback: identity.sign_org_configuration_path)
        else
          redirect_to(identity.sign_org_configuration_path, notice: notice)
        end
      end
