...
module Jit
  module Identity
    module Sign
      module Org
        module In
          class BulletinsController < Jit::Identity::Sign::Org::ApplicationController
            auth_required!
            ...
            def update
              ...
              redirect_to(identity.sign_org_in_bulletin_path(rd: params[:rd], ri: params[:ri]))
            end

            def destroy
              rd_param = params[:rd].presence
              consume_bulletin!
              safe_redirect_to_rd_or_default!(rd_param, default_path: identity.sign_org_root_path(ri: params[:ri]))
            end
