          ...
          class SecretsController < ApplicationController
            ...
            def process_standard_login(staff, secret)
              ...
              if result[:status] == :mfa_required
                ...
                redirect_to(
                  identity.sign_org_in_session_path(ri: params[:ri]),
                  notice: I18n.t(
                ...
                if issue_bulletin!
                  redirect_to(
                    identity.sign_org_in_bulletin_path(rd: params[:rd], ri: params[:ri]),
                    notice: t("sign.org.authentication.secret.create.success"),
                  )
                else
                  safe_redirect_to_rd_or_default!(params[:rd], default_path: identity.sign_org_root_path(ri: params[:ri]))
                end
              else
