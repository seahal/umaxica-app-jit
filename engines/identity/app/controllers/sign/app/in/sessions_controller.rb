...
module Jit::Identity
    class Sign::App::In::SessionsController < Jit::Identity::Sign::App::ApplicationController
      ...
      def destroy
        ...
        if ref.present?
          ...
          session.delete(:pending_login_user_id)
          log_out
          redirect_to(identity.new_sign_app_in_path, notice: I18n.t("sign.app.in.session.cancelled"))
        end
      end
      ...
      def redirect_to_return_path(notice:)
        ...
        if return_path.present?
          flash[:notice] = notice
          jump_to_generated_url(return_path, fallback: identity.sign_app_configuration_path)
        else
          redirect_to(identity.sign_app_configuration_path, notice: notice)
        end
      end
