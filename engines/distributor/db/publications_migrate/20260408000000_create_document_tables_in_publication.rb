...
class CreateDocumentTablesInPublication < ActiveRecord::Migration[8.2]
  ...
  def ensure_document_foreign_keys(prefix)
    ...
    add_foreign_key(versions_table, documents_table, column: "#{prefix}_document_id", validate: false) unless foreign_key_exists?(
      versions_table, documents_table, column: "#{prefix}_document_id",
    )
    add_foreign_key(revisions_table, documents_table, column: "#{prefix}_document_id", validate: false) unless foreign_key_exists?(
      revisions_table, documents_table, column: "#{prefix}_document_id",
    )
    add_foreign_key(documents_table, statuses_table, column: :status_id, validate: false) unless foreign_key_exists?(
      documents_table,
      statuses_table, column: :status_id,
    )
    add_foreign_key(documents_table, versions_table, column: :latest_version_id, validate: false) unless foreign_key_exists?(
      documents_table, versions_table, column: :latest_version_id,
    )
    add_foreign_key(documents_table, revisions_table, column: :latest_revision_id, validate: false) unless foreign_key_exists?(
      documents_table, revisions_table, column: :latest_revision_id,
    )
    add_foreign_key(tag_masters_table, tag_masters_table, column: :parent_id, validate: false) unless foreign_key_exists?(
      tag_masters_table, tag_masters_table, column: :parent_id,
    )
    add_foreign_key(tags_table, tag_masters_table, column: "#{prefix}_document_tag_master_id", validate: false) unless foreign_key_exists?(
      tags_table, tag_masters_table, column: "#{prefix}_document_tag_master_id",
    )
    add_foreign_key(tags_table, documents_table, column: "#{prefix}_document_id", validate: false) unless foreign_key_exists?(
      tags_table,
      documents_table, column: "#{prefix}_document_id",
    )
    add_foreign_key(category_masters_table, category_masters_table, column: :parent_id, validate: false) unless foreign_key_exists?(
      category_masters_table, category_masters_table, column: :parent_id,
    )
    add_foreign_key(
      categories_table, category_masters_table, column: "#{prefix}_document_category_master_id",
                                                validate: false,
    ) unless foreign_key_exists?(
      categories_table, category_masters_table, column: "#{prefix}_document_category_master_id",
    )
    add_foreign_key(categories_table, documents_table, column: "#{prefix}_document_id", validate: false) unless foreign_key_exists?(
      categories_table, documents_table, column: "#{prefix}_document_id",
    )
  end
