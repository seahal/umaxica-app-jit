...
class ComTimelineTagMasterTest < ActiveSupport::TestCase
  test "validates id presence and uniqueness" do
    master = ComTimelineTagMaster.new(id: nil)
    assert_not master.valid?
    assert_not_empty master.errors[:id]

    existing = ComTimelineTagMaster.first || ComTimelineTagMaster.create!(id: "EXISTING")
    duplicate = ComTimelineTagMaster.new(id: existing.id.downcase)
    assert_not duplicate.valid?
    assert_not_empty duplicate.errors[:id]
  end

  test "upcases id before validation" do
    master = ComTimelineTagMaster.new(id: "new_category")
    master.valid?
    assert_equal "NEW_CATEGORY", master.id
  end

  test "validates id format" do
    master = ComTimelineTagMaster.new(id: "INVALID-ID!")
    assert_not master.valid?
    assert_not_empty master.errors[:id]
  end

  test "root? returns true when parent_id is NEYO" do
    master = ComTimelineTagMaster.new(id: "ROOT", parent_id: "NEYO")
  ...
  test "parent and children associations" do
    parent = ComTimelineTagMaster.create!(id: "PARENT_CAT")
    child = ComTimelineTagMaster.create!(id: "CHILD_CAT", parent: parent)
  ...
  test "name returns translated string" do
    master = ComTimelineTagMaster.new(id: "TEST_CAT")
  ...
  test "validates length of id" do
    record = ComTimelineTagMaster.new(id: "A" * 256)
