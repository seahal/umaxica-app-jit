# typed: false
# frozen_string_literal: true

# == Schema Information
#
# Table name: user_activities
# Database name: activity
#
#  id              :bigint           not null, primary key
#  actor_type      :text             default(""), not null
#  context         :jsonb            not null
#  current_value   :text             default(""), not null
#  expires_at      :datetime         not null
#  ip_address      :inet             default(#<IPAddr: IPv4:0.0.0.0/255.255.255.255>), not null
#  occurred_at     :datetime         not null
#  previous_digest :string
#  previous_value  :text             default(""), not null
#  record_digest   :string
#  sequence_number :bigint
#  subject_type    :text             not null
#  tsa_at          :datetime
#  tsa_token       :text
#  created_at      :datetime         not null
#  updated_at      :datetime         not null
#  actor_id        :bigint           default(0), not null
#  event_id        :bigint           default(0), not null
#  level_id        :bigint           default(4), not null
#  subject_id      :bigint           not null
#
# Indexes
#
#  idx_on_subject_type_subject_id_occurred_at_a29eb711dd  (subject_type,subject_id,occurred_at)
#  index_user_activities_on_actor                         (actor_type,actor_id)
#  index_user_activities_on_actor_id_and_occurred_at      (actor_id,occurred_at)
#  index_user_activities_on_chain_validation              (sequence_number,record_digest)
#  index_user_activities_on_event_id                      (event_id)
#  index_user_activities_on_expires_at                    (expires_at)
#  index_user_activities_on_level_id                      (level_id)
#  index_user_activities_on_occurred_at                   (occurred_at)
#  index_user_activities_on_record_digest                 (record_digest) UNIQUE
#  index_user_activities_on_sequence_number               (sequence_number) UNIQUE
#  index_user_activities_on_subject_id                    (subject_id)
#  index_user_activities_on_tsa_at                        (tsa_at)
#
# Foreign Keys
#
#  fk_rails_...  (event_id => user_activity_events.id)
#  fk_rails_...  (level_id => user_activity_levels.id)
#

require "test_helper"

class UserActivityTest < ActiveSupport::TestCase
  fixtures :users, :staffs, :user_activity_events, :user_activity_levels, :user_statuses

  def setup
    @user = users(:one)
    @audit_event = UserActivityEvent.find(UserActivityEvent::LOGIN_SUCCESS)
    @level = UserActivityLevel.find_or_create_by!(id: UserActivityLevel::INFO)
    @audit = UserActivity.create!(
      user: @user,
      user_activity_level: @level,
      user_activity_event: @audit_event,
      timestamp: Time.current,
      ip_address: "192.168.1.1",
    )
  end

  test "uses bigint primary key" do
    assert_kind_of Integer, @audit.id
  end

  test "inherits from ActivityRecord" do
    assert_operator UserActivity, :<, ActivityRecord
  end

  test "ip_address can be stored" do
    assert_equal "192.168.1.1", @audit.ip_address.to_s
  end

  test "requires user" do
    audit = UserActivity.new(
      user_activity_event: @audit_event,
    )

    assert_not audit.valid?
    assert_not_empty audit.errors[:subject_id]
  end

  test "requires user_activity_event" do
    audit = UserActivity.new(
      user: @user,
    )

    # Defaults to NOTHING, so it should be valid
    assert_predicate audit, :valid?
  end

  test "validates foreign key constraint on event_id" do
    audit = UserActivity.new(
      user: @user,
      event_id: 9999,
      timestamp: Time.current,
    )

    assert_not audit.valid?
    assert_not_empty audit.errors[:event_id]
    assert_includes audit.errors[:event_id], "must reference an existing user_activity_event"
  end

  test "belongs to polymorphic actor" do
    association = UserActivity.reflect_on_association(:actor)

    assert_not_nil association
    assert_equal :belongs_to, association.macro
    assert_predicate association, :polymorphic?
  end

  test "can be created with a User as actor" do
    actor_user = users(:one)
    audit = UserActivity.create!(
      user: @user,
      user_activity_event: @audit_event,
      actor: actor_user,
    )

    assert_equal actor_user.id, audit.actor_id
    assert_equal "User", audit.actor_type
    assert_equal actor_user, audit.actor
  end

  test "can be created with a Staff as actor" do
    actor_staff = staffs(:one)
    audit = UserActivity.create!(
      user: @user,
      user_activity_event: @audit_event,
      actor: actor_staff,
    )

    assert_equal actor_staff.id, audit.actor_id
    assert_equal "Staff", audit.actor_type
    assert_equal actor_staff, audit.actor
  end

  test "User and Staff can both be actors in different audits" do
    actor_user = users(:one)
    actor_staff = staffs(:one)

    # Multiple audits for the same user can have different actors
    UserActivity.create!(
      user: @user,
      user_activity_event: @audit_event,
      actor: actor_user,
    )

    UserActivity.create!(
      user: @user,
      user_activity_event: @audit_event,
      actor: actor_staff,
    )

    # Retrieve multiple audits related to the same User
    user_actors = @user.user_activities.where(actor_type: "User")
    staff_actors = @user.user_activities.where(actor_type: "Staff")

    assert_not_empty user_actors
    assert_not_empty staff_actors
  end

  # Additional tests for helper methods
  test "user helper method returns user" do
    assert_equal @user, @audit.user
  end

  test "user_id helper method returns user id" do
    assert_equal @user.id.to_s, @audit.user_id
  end

  test "occurred_at alias works" do
    assert_equal @audit.timestamp, @audit.occurred_at
  end

  test "set_timestamp defaults to current time" do
    audit = UserActivity.new(user_activity_event: @audit_event)
    audit.user = @user
    audit.save!

    assert_not_nil audit.timestamp
  end

  test "actor defaults to null user if blank" do
    # When creating without actor
    audit = UserActivity.create!(
      user: @user,
      user_activity_event: @audit_event,
      actor: nil,
    )

    assert_equal 0, audit.actor_id
    assert_equal "User", audit.actor_type
  end

  test "event_id rejects negative values" do
    audit = UserActivity.new(
      user: @user,
      event_id: -1,
      timestamp: Time.current,
    )

    assert_not audit.valid?
    assert_not_empty audit.errors[:event_id]
  end

  test "event_id rejects decimal values" do
    audit = UserActivity.new(
      user: @user,
      event_id: 1.5,
      timestamp: Time.current,
    )

    assert_not audit.valid?
    assert_not_empty audit.errors[:event_id]
  end

  test "level_id rejects negative values" do
    audit = UserActivity.new(
      user: @user,
      level_id: -1,
      timestamp: Time.current,
    )

    assert_not audit.valid?
    assert_not_empty audit.errors[:level_id]
  end

  test "level_id rejects decimal values" do
    audit = UserActivity.new(
      user: @user,
      level_id: 1.5,
      timestamp: Time.current,
    )

    assert_not audit.valid?
    assert_not_empty audit.errors[:level_id]
  end

  test "event_id accepts zero" do
    audit = UserActivity.new(
      user: @user,
      user_activity_event: @audit_event,
      event_id: 0,
      timestamp: Time.current,
    )

    # Should be valid based on presence/numericality, but may fail reference validation
    assert_equal 0, audit.event_id
  end
end
