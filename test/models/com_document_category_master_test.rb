...
class ComDocumentCategoryMasterTest < ActiveSupport::TestCase
  test "validates id presence and uniqueness" do
    master = ComDocumentCategoryMaster.new(id: nil)
    assert_not master.valid?
    assert_not_empty master.errors[:id]

    existing = ComDocumentCategoryMaster.first || ComDocumentCategoryMaster.create!(id: "EXISTING")
    duplicate = ComDocumentCategoryMaster.new(id: existing.id.downcase)
    assert_not duplicate.valid?
    assert_not_empty duplicate.errors[:id]
  end

  test "upcases id before validation" do
    master = ComDocumentCategoryMaster.new(id: "new_category")
    master.valid?
    assert_equal "NEW_CATEGORY", master.id
  end

  test "validates id format" do
    master = ComDocumentCategoryMaster.new(id: "INVALID-ID!")
    assert_not master.valid?
    assert_not_empty master.errors[:id]
  end

  test "root? returns true when parent_id is NEYO" do
    master = ComDocumentCategoryMaster.new(id: "ROOT", parent_id: "NEYO")
  ...
  test "parent and children associations" do
    parent = ComDocumentCategoryMaster.create!(id: "PARENT_CAT")
    child = ComDocumentCategoryMaster.create!(id: "CHILD_CAT", parent: parent)
  ...
  test "name returns translated string" do
    master = ComDocumentCategoryMaster.new(id: "TEST_CAT")
  ...
  test "validates length of id" do
    record = ComDocumentCategoryMaster.new(id: "A" * 256)
