...
class StaffIdentityAuditStatusTest < ActiveSupport::TestCase
  def setup
    @audit_status = StaffIdentityAuditStatus.new(id: "TEST_EVENT")
  end

  # ID default value tests
  test "default id value is NONE" do
    status = StaffIdentityAuditStatus.new

    assert_equal "NONE", status.id
  end
  ...
  test "id must be present" do
    ...
    assert_not @audit_status.valid?
    assert_not_empty @audit_status.errors[:id]
  end
  ...
  test "id must be unique" do
    StaffIdentityAuditStatus.create!(id: "UNIQUE_EVENT")
    duplicate = StaffIdentityAuditStatus.new(id: "UNIQUE_EVENT")
  ...
  test "id with numbers is invalid" do
    @audit_status.id = "TEST_EVENT_123"
  ...
  test "valid id format with uppercase and underscores" do
    @audit_status.id = "VALID_EVENT_NAME"

    assert_predicate @audit_status, :valid?
  end
  ...
  test "id exceeding 255 characters should raise error when saving" do
    ...
    assert_raises(ActiveRecord::StatementInvalid) do
      StaffIdentityAuditStatus.create!(id: long_id)
    end
  end
end
