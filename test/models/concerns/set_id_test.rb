...
class SetIdTest < ActiveSupport::TestCase
  ...
  test "generated id should be UUIDv7 format" do
    email = UserIdentityEmail.create!(address: "uuid@example.com", confirm_policy: true)
  ...
  test "should use SecureRandom.uuid_v7" do
    ...
    3.times do |i|
      email = UserIdentityEmail.create!(address: "test#{i}@example.com", confirm_policy: true)
  ...
  test "should generate different ids for each record" do
    email1 = UserIdentityEmail.create!(address: "unique1@example.com", confirm_policy: true)
    email2 = UserIdentityEmail.create!(address: "unique2@example.com", confirm_policy: true)

    assert_not_equal email1.id, email2.id
  end

  test "should only generate id before create, not update" do
    email = UserIdentityEmail.create!(address: "update@example.com", confirm_policy: true)
