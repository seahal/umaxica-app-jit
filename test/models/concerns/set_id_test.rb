...
class SetIdTest < ActiveSupport::TestCase
  ...
  test "should generate UUIDv7 before create" do
    email = UserEmail.new(address: "test@example.com", confirm_policy: true, user: @user)
  ...
  test "generated id should be UUIDv7 format" do
    email = UserEmail.create!(address: "uuid@example.com", confirm_policy: true, user: @user)
  ...
  test "should use SecureRandom.uuid_v7" do
    ...
    3.times do |i|
      email = UserEmail.create!(address: "test#{i}@example.com", confirm_policy: true, user: @user)
  ...
  test "should generate different ids for each record" do
    email1 = UserEmail.create!(address: "unique1@example.com", confirm_policy: true, user: @user)
    email2 = UserEmail.create!(address: "unique2@example.com", confirm_policy: true, user: @user)

    assert_not_equal email1.id, email2.id
  end

  test "should only generate id before create, not update" do
    email = UserEmail.create!(address: "update@example.com", confirm_policy: true, user: @user)
