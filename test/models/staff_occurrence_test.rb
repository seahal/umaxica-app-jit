...
class StaffOccurrenceTest < ActiveSupport::TestCase
  ...
  test "status_id rejects negative values" do
    record = build_occurrence(StaffOccurrence, body: "test-body-123", status_id: -1)

    assert_invalid_attribute(record, :status_id)
  end

  test "status_id rejects decimal values" do
    record = build_occurrence(StaffOccurrence, body: "test-body-123", status_id: 1.5)

    assert_invalid_attribute(record, :status_id)
  end

  test "status_id accepts zero" do
    record = build_occurrence(StaffOccurrence, body: "test-body-123", status_id: 0)
