...
class EmailPreferenceRequestTest < ActiveSupport::TestCase
  test "defaults optional preferences" do
    ...
    assert request.preferences_with_defaults[:product_updates]
    assert request.preferences_with_defaults[:promotional_messages]
  end
  ...
  test "mark_preferences! normalizes and marks token used" do
    ...
    assert_not request.preferences["product_updates"]
    assert request.preferences["promotional_messages"]
  ...
  test "token_valid? is false after expiry or use" do
    ...
    assert_predicate request, :token_valid?
