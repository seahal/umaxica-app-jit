...
class AppTimelineTagMasterTest < ActiveSupport::TestCase
  test "validates id presence and uniqueness" do
    master = AppTimelineTagMaster.new(id: nil)
    assert_not master.valid?
    assert_not_empty master.errors[:id]

    existing = AppTimelineTagMaster.first || AppTimelineTagMaster.create!(id: "EXISTING")
    duplicate = AppTimelineTagMaster.new(id: existing.id.downcase)
    assert_not duplicate.valid?
    assert_not_empty duplicate.errors[:id]
  end

  test "upcases id before validation" do
    master = AppTimelineTagMaster.new(id: "new_category")
    master.valid?
    assert_equal "NEW_CATEGORY", master.id
  end

  test "validates id format" do
    master = AppTimelineTagMaster.new(id: "INVALID-ID!")
    assert_not master.valid?
    assert_not_empty master.errors[:id]
  end

  test "root? returns true when parent_id is NEYO" do
    master = AppTimelineTagMaster.new(id: "ROOT", parent_id: "NEYO")
  ...
  test "parent and children associations" do
    parent = AppTimelineTagMaster.create!(id: "PARENT_CAT")
    child = AppTimelineTagMaster.create!(id: "CHILD_CAT", parent: parent)
  ...
  test "name returns translated string" do
    master = AppTimelineTagMaster.new(id: "TEST_CAT")
  ...
  test "validates length of id" do
    record = AppTimelineTagMaster.new(id: "A" * 256)
