import {describe, expect, test} from "bun:test";

type Listener = (...args: unknown[]) => void;

const createDocumentStub = () => {
	const listeners = new Map<string, Listener[]>();

	const addEventListener = (
		event: string,
		handler: Listener,
		_options?: unknown,
	) => {
		const existing = listeners.get(event) ?? [];
		existing.push(handler);
		listeners.set(event, existing);
	};

	const removeEventListener = (event: string, handler: Listener) => {
		const existing = listeners.get(event);
		if (!existing) {
			return;
		}

		listeners.set(
			event,
			existing.filter((candidate) => candidate !== handler),
		);
	};

	return {
		document: {
			readyState: "loading",
			addEventListener,
			removeEventListener,
			getElementById: () => null,
		},
		listeners,
	};
};

describe("Root landing bootstrap", () => {
	test("registers lifecycle events without immediately mounting", async () => {
		const globalAny = globalThis as Record<string, unknown>;
		const originalDocument = globalAny.document;
		const { document, listeners } = createDocumentStub();

		globalAny.document = document;

		try {
			await import("../../../../app/javascript/root/app/landing.tsx");

			const domContentLoadedHandlers = listeners.get("DOMContentLoaded") ?? [];
			expect(domContentLoadedHandlers.length).toBe(2);

			domContentLoadedHandlers[1]?.();

			expect(listeners.get("turbo:load")?.length).toBe(1);
			expect(listeners.get("turbo:before-render")?.length).toBe(1);
		} finally {
			if (originalDocument === undefined) {
				delete globalAny.document;
			} else {
				globalAny.document = originalDocument;
			}
		}
	});
});
