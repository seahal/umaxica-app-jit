...
class Docs::Org::ApplicationHelperTest < ActionView::TestCase
  setup do
    extend Docs::Org::ApplicationHelper
  end
  ...
  test "title_generator returns NAME when title blank" do
    ...
    assert_equal "DocsOrg", blank_result
    assert_equal "DocsOrg", title_generator("")
  ensure
  ...
  test "title_generator concatenates title with NAME when present" do
    ...
    assert_equal "Docs Admin | DocsOrg", title_generator("Docs Admin")
  ensure
    original_name.nil? ? ENV.delete("NAME") : ENV["NAME"] = original_name
  end
end
