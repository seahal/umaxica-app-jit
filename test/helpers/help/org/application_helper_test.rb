...
class Help::Org::ApplicationHelperTest < ActionView::TestCase
  setup do
    extend Help::Org::ApplicationHelper
  end
  ...
  test "title_generator returns NAME when title blank" do
    ...
    assert_equal "HelpOrg", blank_result
    assert_equal "HelpOrg", title_generator("")
  ensure
  ...
  test "title_generator concatenates title with NAME when present" do
    ...
    assert_equal "Support Admin | HelpOrg", title_generator("Support Admin")
  ensure
    original_name.nil? ? ENV.delete("NAME") : ENV["NAME"] = original_name
  end
end
