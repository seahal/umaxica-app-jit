...
class News::Org::ApplicationHelperTest < ActionView::TestCase
  setup do
    extend News::Org::ApplicationHelper
  end
  ...
  test "title_generator returns NAME when title blank" do
    ...
    assert_equal "NewsOrg", blank_result
    assert_equal "NewsOrg", title_generator("")
  ensure
  ...
  test "title_generator concatenates title with NAME when present" do
    ...
    assert_equal "Internal News | NewsOrg", title_generator("Internal News")
  ensure
    original_name.nil? ? ENV.delete("NAME") : ENV["NAME"] = original_name
  end
end
