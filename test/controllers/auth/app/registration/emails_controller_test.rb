...
class Auth::App::Registration::EmailsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "includes navigation links to other registration flows" do
    ...
    assert_select "a[href=?]", new_auth_app_registration_path, count: 0
    assert_select "a[href=?]", new_auth_app_authentication_email_path, count: 0
  end

  test "edit redirects to new when email record not found" do
    get edit_auth_app_registration_email_url(id: "non-existent-id"), headers: default_headers

    assert_response :redirect
    assert_includes response.location, new_auth_app_registration_email_path
  end
  ...
  test "i18n flash messages for email registration flow exist" do
    ...
    session_expired_key = "auth.app.registration.email.edit.session_expired"
    create_key = "auth.app.registration.email.create.verification_code_sent"
    update_key = "auth.app.registration.email.update.success"
  ...
  test "rejects wrong OTP codes with error message" do
    ...
    # Attempt wrong code
    patch auth_app_registration_email_url(user_email.id),
          params: {
            id: user_email.id,
            user_identity_email: {
              pass_code: "000000"
  ...
  test "telephone i18n flash messages exist" do
    ...
    session_expired_key = "auth.app.registration.telephone.edit.session_expired"
    create_key = "auth.app.registration.telephone.create.verification_code_sent"
    update_key = "auth.app.registration.telephone.update.success"
  ...
  test "turnstile validation error message i18n key exists" do
    ...
    assert_not_nil I18n.t("auth.app.registration.email.create.turnstile_validation_failed", locale: :ja, default: nil)
    assert_not_nil I18n.t("auth.app.registration.email.create.turnstile_validation_failed", locale: :en, default: nil)
  end
  ...
  test "redirects to root when user is already logged in" do
    ...
    assert_equal I18n.t("auth.app.registration.email.already_logged_in"), flash[:alert]
  end
