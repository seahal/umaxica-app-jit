...
class Top::App::Preference::RegionsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "edit form contains region language and timezone sections with proper structure" do
    ...
    assert_select "form[method='post']" do
      ...
      assert_select ".language-section" do
        assert_select "h2", text: I18n.t("top.app.preferences.regions.language_section")
        assert_select ".language-selection label[for='language']", text: I18n.t("top.app.preferences.regions.select_language")
      ...
      assert_select ".timezone-section" do
        assert_select "h2", text: I18n.t("top.app.preferences.regions.timezone_section")
        assert_select ".timezone-selection label[for='timezone']", text: I18n.t("top.app.preferences.regions.select_timezone")
  ...
  test "edit form preselects options matching session values" do
    patch top_app_preference_region_url, params: { region: "US", language: "EN", timezone: "Asia/Tokyo" }
    follow_redirect!
  ...
  test "PATCH with multiple params updates session and redirects with success notice" do
    patch top_app_preference_region_url, params: { region: "US", country: "US", language: "EN", timezone: "Asia/Tokyo" }

    assert_redirected_to edit_top_app_preference_region_url(lx: "en", ri: "us", tz: "asia/tokyo")
  ...
  test "PATCH with language normalizes to uppercase and stores in session" do
    patch top_app_preference_region_url, params: { language: "ja" }
  ...
  test "PATCH with unsupported language returns unprocessable entity" do
    patch top_app_preference_region_url, params: { language: "invalid" }

    assert_response :unprocessable_content
  end

  test "PATCH with timezone stores timezone identifier in session" do
    patch top_app_preference_region_url, params: { timezone: "Asia/Tokyo" }

    assert_redirected_to edit_top_app_preference_region_url(lx: "ja", ri: "jp", tz: "asia/tokyo")
    assert_equal "Asia/Tokyo", session[:timezone]
  end

  test "PATCH with invalid timezone returns unprocessable entity" do
    patch top_app_preference_region_url, params: { timezone: "Invalid/Timezone" }
  ...
  test "GET edit with lx parameter preselects language without saving to session" do
    get edit_top_app_preference_region_url(lx: "en")
  ...
  test "GET edit with tz parameter preselects timezone without saving to session" do
    get edit_top_app_preference_region_url(tz: "utc")
  ...
  test "GET edit with multiple URL parameters preselects all options" do
    get edit_top_app_preference_region_url(lx: "en", ri: "us", tz: "utc")
  ...
  test "URL parameters take precedence over session values" do
    # Set session values
    patch top_app_preference_region_url, params: { region: "JP", language: "JA", timezone: "Asia/Tokyo" }
    follow_redirect!
  ...
  test "URL parameter lx normalizes ja to JA for display" do
    get edit_top_app_preference_region_url(lx: "ja")
  ...
  test "URL parameter tz normalizes jst to Asia/Tokyo for display" do
    get edit_top_app_preference_region_url(tz: "jst")
  ...
  test "URL parameter tz normalizes kst to Asia/Seoul for display" do
    get edit_top_app_preference_region_url(tz: "kst")
  ...
  test "cookie should be set after updating preferences" do
    patch top_app_preference_region_url, params: { region: "US", language: "EN" }
  ...
  test "multiple preference updates should maintain cookie consistency" do
    ...
    # Second update
    patch top_app_preference_region_url, params: { language: "EN" }
  ...
  test "preferences should persist across multiple updates" do
    patch top_app_preference_region_url, params: { timezone: "Etc/UTC" }
