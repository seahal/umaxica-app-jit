...
class Apex::Com::Web::V1::CookieControllerTest < ActionDispatch::IntegrationTest
  setup do
    host! ENV.fetch("APEX_CORPORATE_URL", "com.localhost")
  end

  test "GET show returns show_banner true when consent is false" do
    controller = Apex::Com::Web::V1::CookiesController
    ...
    controller.any_instance.stub(:decode_and_verify_preference_jwt, { "preferences" => { "consent" => false } }) do
      get apex_com_web_v1_cookie_path, as: :json
    end
  ...
  test "PATCH update returns 200 and sets jit_preference_consented cookie with com domain" do
    ...
    with_env("COOKIE_DOMAIN_COM", ".com.example.test") do
      controller.any_instance.stub(
        ...
        controller.any_instance.stub(:refresh_token_expires_at, expires_at) do
          patch apex_com_web_v1_cookie_path, as: :json
        end
      end
    end
  ...
  test "PATCH update with consented true updates com preference cookie and issues access token" do
    ...
    controller.any_instance.stub(
      ...
      patch apex_com_web_v1_cookie_path, params: { consented: true }, as: :json
    end
