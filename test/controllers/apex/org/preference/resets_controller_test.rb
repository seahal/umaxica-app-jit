...
class Apex::Org::Preference::ResetsControllerTest < ActionDispatch::IntegrationTest
  setup do
    host! ENV.fetch("APEX_SERVICE_URL", "org.localhost")
    https!
  ...
  test "should destroy and redirect" do
    ...
    OrgPreferenceAudit.count

    delete apex_org_preference_reset_url, headers: { "Cookie" => "#{cookie_name}=#{@token}" }

    assert_response :redirect
    # Redirect location might include query params for defaults
    assert_match edit_apex_org_preference_reset_path, response.location
    ...
    audits = OrgPreferenceAudit.where(event_id: "RESET_BY_USER_DECISION").where(created_at: 5.seconds.ago..)
  ...
  test "should get edit" do
    cookie_name = Rails.env.production? ? "__Secure-Jit-Preference" : "Jit-Preference"
    get edit_apex_org_preference_reset_url, headers: { "Cookie" => "#{cookie_name}=#{@token}" }
