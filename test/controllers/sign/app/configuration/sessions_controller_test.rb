...
class Sign::App::Configuration::SessionsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "index includes link to sign out current session" do
  ...
  test "index shows turbo confirmation for revoke" do
    UserToken.create!(
      user_id: @user.id,
      public_id: "conf#{SecureRandom.hex(4)}",
      refresh_expires_at: 1.day.from_now,
      user_token_kind_id: "BROWSER_WEB",
    )
    ...
    assert_response :success
    assert_select "button[data-turbo-confirm]", minimum: 1
  end
end
