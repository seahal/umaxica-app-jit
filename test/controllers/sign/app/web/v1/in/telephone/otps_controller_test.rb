...
class Sign::App::Web::V1::In::Telephone::OtpsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "recent issued returns 429 with retry_after and header" do
    ...
    post sign_app_web_v1_in_telephone_otp_path,
  ...
  test "success rotates OTP so only latest code verifies" do
    ...
    assert_not verifier.verify_otp_code(telephone_record, old_code)[:success]
    assert verifier.verify_otp_code(telephone_record, new_code)[:success]
  end
