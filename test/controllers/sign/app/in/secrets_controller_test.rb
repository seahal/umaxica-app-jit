...
class Sign::App::In::SecretsControllerTest < ActionDispatch::IntegrationTest
  setup do
    ...
    @user_secret, @password = UserSecret.issue!(
      name: "Login Password",
      user_id: @user.id,
      user_secret_kind_id: "LOGIN", # UserSecretKind::LOGIN might not be loaded if class not referenced, but string usually works or reference constant
      uses: 100,
    )
  end
  ...
  def default_headers
    { "Host" => ENV["SIGN_SERVICE_URL"] || "sign.app.localhost", "HTTPS" => "on" }
  end
end
