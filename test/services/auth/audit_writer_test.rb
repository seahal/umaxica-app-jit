...
module Auth
  class AuditWriterTest < ActiveSupport::TestCase
    ...
    test "write returns true on success" do
      ...
      assert result
      assert UserActivity.exists?(event_id: UserActivityEvent::LOGGED_IN, subject_id: @user.id)
    end
    ...
    test "write returns false on failure" do
      ...
      ActivityRecord.connected_to(role: :writing) do
        UserActivityEvent.where(id: invalid_event_id).delete_all
      end
      ...
      assert_not UserActivity.exists?(event_id: invalid_event_id, subject_id: @user.id),
