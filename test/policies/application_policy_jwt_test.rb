...
class ApplicationPolicyJwtTest < ActiveSupport::TestCase
  ...
  def test_jwt_scopes_returns_empty_array_when_no_token
    policy = ApplicationPolicy.new(user: nil, user: TestRecord.new(1))
  ...
  def test_combining_jwt_and_db_check_example
    ...
    record_with_user_id = Struct.new(:user_id).new(user.id)
    policy = ApplicationPolicy.new(user: user, user: record_with_user_id)
