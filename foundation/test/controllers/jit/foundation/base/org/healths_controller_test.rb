      ...
      module Org
        class HealthsControllerTest < ActionDispatch::IntegrationTest
          ...
          def with_stubbed_health_status(result)
            Base::Org::HealthsController.send(:define_method, :get_status) { result }
            yield
          ensure
            if Base::Org::HealthsController.private_method_defined?(:get_status) ||
                Base::Org::HealthsController.method_defined?(:get_status, false)
              Base::Org::HealthsController.send(:remove_method, :get_status)
            end
          end
        end
      end
