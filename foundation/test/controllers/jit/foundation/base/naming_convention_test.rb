...
# frozen_string_literal: true
    ...
    class CoreNamingConventionTest < ActiveSupport::TestCase
      STATION_CONTROLLER_ROOT = Rails.root.join("app/controllers/core").freeze
      STATION_TEST_ROOT = Rails.root.join("test/controllers/core").freeze

      test "all core controllers follow naming convention" do
        core_controllers = Dir.glob(STATION_CONTROLLER_ROOT.join("**/*.rb"))
      ...
      test "all core controller tests match their controllers" do
        core_controllers = Dir.glob(STATION_CONTROLLER_ROOT.join("**/*_controller.rb"))

        core_controllers.each do |controller_path|
          relative_path = Pathname.new(controller_path).relative_path_from(STATION_CONTROLLER_ROOT).to_s
          ...
          flat_filename = path_without_domain.tr("/", "_").sub("_controller.rb", "_controller_test.rb")
