...
class CreateStaffIdentityAuditLevels < ActiveRecord::Migration[8.2]
  def change
    ...
    safety_assured { add_index(:staff_identity_audits, :level_id) } unless index_exists?(:staff_identity_audits, :level_id)
    safety_assured { add_foreign_key(:staff_identity_audits, :staff_identity_audit_levels, column: :level_id) } unless foreign_key_exists?(
