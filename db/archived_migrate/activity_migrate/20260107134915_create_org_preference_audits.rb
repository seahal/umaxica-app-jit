...
class CreateOrgPreferenceAudits < ActiveRecord::Migration[8.2]
  def change
    ...
    add_index(
      :org_preference_audits, %i(subject_type subject_id occurred_at),
      name: "idx_on_subject_type_subject_id_occurred_at_org_pref",
    )
    ...
    add_index(:org_preference_audits, :occurred_at)
    add_index(:org_preference_audits, :expires_at)
  end
end
