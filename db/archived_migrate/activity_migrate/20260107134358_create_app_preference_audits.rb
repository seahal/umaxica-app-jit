...
class CreateAppPreferenceAudits < ActiveRecord::Migration[8.2]
  def change
    create_table(:app_preference_audits) do |t|
    ...
    add_index(
      :app_preference_audits, %i(subject_type subject_id occurred_at),
      name: "idx_on_subject_type_subject_id_occurred_at_app_pref",
    )
