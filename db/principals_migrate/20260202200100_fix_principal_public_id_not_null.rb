  ...
  def up
    safety_assured do
      ...
      if table_exists?(:users) && column_exists?(:users, :public_id)
        execute "UPDATE users SET public_id = gen_random_uuid()::text WHERE public_id IS NULL OR public_id = ''"
      ...
      if table_exists?(:user_one_time_passwords) && column_exists?(:user_one_time_passwords, :public_id)
        execute "UPDATE user_one_time_passwords SET public_id = gen_random_uuid()::text WHERE public_id IS NULL"
