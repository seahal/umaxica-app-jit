...
class FixNewsConsistency < ActiveRecord::Migration[8.2]
  def change
    %i(org com app).each do |prefix|
      ...
      add_foreign_key(
        "#{prefix}_timeline_tag_masters", "#{prefix}_timeline_tag_masters", column: :parent_id,
                                                                            if_not_exists: true,
      )
      ...
      add_foreign_key(
        "#{prefix}_timeline_category_masters", "#{prefix}_timeline_category_masters", column: :parent_id,
                                                                                      if_not_exists: true,
      )
