...
class ConvertPreferencePks < ActiveRecord::Migration[8.0]
  def up
    ...
    prefixes.each do |prefix|
      suffixes.each do |suffix|
        add_foreign_key :"#{prefix}_preference_#{suffix}", :"#{prefix}_preferences", column: :preference_id, validate: false
      end
      ...
      add_foreign_key :"#{prefix}_preference_regions", :"#{prefix}_preference_region_options", column: :option_id, primary_key: :id, validate: false
      add_foreign_key :"#{prefix}_preference_timezones", :"#{prefix}_preference_timezone_options", column: :option_id, primary_key: :id, validate: false
      add_foreign_key :"#{prefix}_preference_languages", :"#{prefix}_preference_language_options", column: :option_id, primary_key: :id, validate: false
      add_foreign_key :"#{prefix}_preference_colorthemes", :"#{prefix}_preference_colortheme_options", column: :option_id, primary_key: :id, validate: false
    end
  end
