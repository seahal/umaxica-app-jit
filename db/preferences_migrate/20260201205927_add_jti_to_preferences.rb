...
class AddJtiToPreferences < ActiveRecord::Migration[8.2]
  ...
  def up
    ...
    add_index(:app_preferences, :jti, unique: true, algorithm: :concurrently) unless index_exists?(
      :app_preferences,
      :jti,
    )
    add_index(:com_preferences, :jti, unique: true, algorithm: :concurrently) unless index_exists?(
      :com_preferences,
      :jti,
    )
    add_index(:org_preferences, :jti, unique: true, algorithm: :concurrently) unless index_exists?(
      :org_preferences,
      :jti,
    )
  end
