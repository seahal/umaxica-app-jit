...
class AddDeviceIdToTokens < ActiveRecord::Migration[8.2]
  ...
  def change
    add_column(:user_tokens, :device_id, :string, null: false, default: "") unless column_exists?(
      :user_tokens,
      :device_id,
    )
    add_column(:staff_tokens, :device_id, :string, null: false, default: "") unless column_exists?(
      :staff_tokens,
      :device_id,
    )
