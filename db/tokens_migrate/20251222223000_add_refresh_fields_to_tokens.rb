...
class AddRefreshFieldsToTokens < ActiveRecord::Migration[8.2]
  ...
  def up
    ...
    change_column_null(:staff_tokens, :refresh_token_digest, true) if column_exists?(
      :staff_tokens,
  ...
  def down
    ...
    add_index(:user_tokens, :refresh_token_digest, unique: true) unless index_exists?(
      :user_tokens,
      :refresh_token_digest,
    )
    add_index(:staff_tokens, :refresh_token_digest, unique: true) unless index_exists?(
      :staff_tokens,
      :refresh_token_digest,
    )

    change_column_null(:user_tokens, :refresh_token_digest, false) if column_exists?(
      :user_tokens,
      :refresh_token_digest,
    )
    change_column_null(:staff_tokens, :refresh_token_digest, false) if column_exists?(
      :staff_tokens,
      :refresh_token_digest,
    )
  end
