...
class RemoveRedundantDocumentIndexes < ActiveRecord::Migration[8.2]
  def change
    reversible do |dir|
      ...
      dir.down do
        add_index(
          :org_document_versions, :org_document_id,
          name: "index_org_document_versions_on_org_document_id",
          if_not_exists: true,
        )
        add_index(
          :org_document_tags, :org_document_id,
          name: "index_org_document_tags_on_org_document_id",
          if_not_exists: true,
        )

        add_index(
          :com_document_versions, :com_document_id,
          name: "index_com_document_versions_on_com_document_id",
          if_not_exists: true,
        )
        add_index(
          :com_document_tags, :com_document_id,
          name: "index_com_document_tags_on_com_document_id",
          if_not_exists: true,
        )

        add_index(
          :app_document_versions, :app_document_id,
          name: "index_app_document_versions_on_app_document_id",
          if_not_exists: true,
        )
        add_index(
          :app_document_tags, :app_document_id,
          name: "index_app_document_tags_on_app_document_id",
          if_not_exists: true,
        )
      end
