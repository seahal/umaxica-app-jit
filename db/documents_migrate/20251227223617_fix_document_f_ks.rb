...
class FixDocumentFKs < ActiveRecord::Migration[8.2]
  def change
    %i(org com app).each do |prefix|
      ...
      remove_foreign_key(
        "#{prefix}_document_tag_masters", "#{prefix}_document_tag_masters", column: :parent_id,
                                                                            if_exists: true,
      )
      add_foreign_key(
        "#{prefix}_document_tag_masters", "#{prefix}_document_tag_masters", column: :parent_id,
      ...
      remove_foreign_key(
        "#{prefix}_document_category_masters", "#{prefix}_document_category_masters",
        column: :parent_id, if_exists: true,
      )
      add_foreign_key(
        "#{prefix}_document_category_masters", "#{prefix}_document_category_masters", column: :parent_id,
                                                                                      validate: false,
      )
