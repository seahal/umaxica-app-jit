class ChristmasGuestCleanup < ActiveRecord::Migration[8.2]
  def change
    ...
    # Com
    set_defaults_and_nulls(:com_contact_emails, string_cols: [ :token_digest, :verifier_digest, :hotp_secret ], time_cols: [ :token_expires_at, :verifier_expires_at ], int_cols: [ :hotp_counter ])
