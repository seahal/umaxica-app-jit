...
class AddGuestIndexesAndFks < ActiveRecord::Migration[8.2]
  def change
    ...
    add_index(
      :org_contact_statuses, "lower(id)", unique: true, name: "index_org_contact_statuses_on_lower_id",
                                          if_not_exists: true,
    )
    add_index(
      :org_contact_categories, "lower(id)", unique: true, name: "index_org_contact_categories_on_lower_id",
                                            if_not_exists: true,
    )
    add_index(
      :com_contact_statuses, "lower(id)", unique: true, name: "index_com_contact_statuses_on_lower_id",
                                          if_not_exists: true,
    )
    add_index(
      :com_contact_categories, "lower(id)", unique: true, name: "index_com_contact_categories_on_lower_id",
                                            if_not_exists: true,
    )
    add_index(
      :app_contact_statuses, "lower(id)", unique: true, name: "index_app_contact_statuses_on_lower_id",
                                          if_not_exists: true,
    )
    add_index(
      :app_contact_categories, "lower(id)", unique: true, name: "index_app_contact_categories_on_lower_id",
                                            if_not_exists: true,
    )
