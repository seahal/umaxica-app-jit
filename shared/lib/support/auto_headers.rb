  ...
  setup do
    unless instance_variable_defined?(:@headers) && @headers.present?
      if instance_variable_defined?(:@user) && @user
        host = defined?(ENV) ? (ENV["IDENTITY_SIGN_APP_URL"] || "sign.app.localhost") : "sign.app.localhost"
        @headers = as_user_headers(@user, host: host)
      elsif instance_variable_defined?(:@staff) && @staff
        host = defined?(ENV) ? (ENV["IDENTITY_SIGN_ORG_URL"] || "sign.org.localhost") : "sign.org.localhost"
