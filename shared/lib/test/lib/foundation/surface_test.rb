...
class CoreSurfaceTest < ActiveSupport::TestCase
  ...
  test "current delegates to detect" do
    ...
    assert_equal :app, Base::Surface.current(request)
  end

  test "matches returns true when surface matches" do
    request = MockRequest.new("app.example.com")

    assert Base::Surface.matches?(request, :app)
  end

  test "matches returns false when surface does not match" do
    request = MockRequest.new("app.example.com")

    assert_not Base::Surface.matches?(request, :org)
  end

  test "normalized_host lowercases and removes trailing dot" do
    assert_equal "example.com", Base::Surface.send(:normalized_host, "EXAMPLE.COM")
    assert_equal "example.com", Base::Surface.send(:normalized_host, "example.com.")
  end

  test "normalized_host removes port" do
    assert_equal "example.com", Base::Surface.send(:normalized_host, "example.com:8080")
  end

  test "normalized_host extracts host from a URL string" do
    assert_equal "app.example.com", Base::Surface.send(:normalized_host, "https://APP.EXAMPLE.COM:3000/path")
  end

  test "normalized_host returns nil for blank" do
    assert_nil Base::Surface.send(:normalized_host, "")
    assert_nil Base::Surface.send(:normalized_host, nil)
  end

  test "extract_host returns host if request responds to host" do
    request = MockRequest.new("app.example.com")

    assert_equal "app.example.com", Base::Surface.send(:extract_host, request)
  end

  test "extract_host converts string to host" do
    assert_equal "app.example.com", Base::Surface.send(:extract_host, "app.example.com")
  end
end
