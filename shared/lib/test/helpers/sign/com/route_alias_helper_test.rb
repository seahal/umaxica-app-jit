...
module Sign
  module Com
    class RouteAliasHelperTest < ActiveSupport::TestCase
      ...
      test "defines acme app-to-com route aliases" do
        instance = test_helper_instance
        app_methods = instance.methods.grep(/^acme_app_/)
        com_methods = instance.methods.grep(/^acme_com_/)

        found = false
        app_methods.each do |helper_name|
          expected_com_name = helper_name.to_s.sub("acme_app_", "acme_com_").to_sym
          next unless com_methods.include?(expected_com_name)
        ...
        assert found, "Expected at least one acme app-to-com route alias to be defined"
      end
