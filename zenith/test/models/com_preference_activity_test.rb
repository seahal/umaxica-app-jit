...
class ComPreferenceActivityTest < ActiveSupport::TestCase
  ...
  setup do
    @audit = com_preference_activities(:one)
  ...
  test "belongs to com_preference_activity_level" do
    assert_equal com_preference_activity_levels(:info), @audit.com_preference_activity_level
  end

  test "belongs to com_preference_activity_event" do
    assert_equal com_preference_activity_events(:create_new_preference_token), @audit.com_preference_activity_event
  end

  test "rejects unknown event_id before database foreign key enforcement" do
    audit = ComPreferenceActivity.new(
      subject_id: @preference.id,
      subject_type: "ComPreference",
      occurred_at: Time.current,
      expires_at: 1.year.from_now,
      event_id: 999_999,
      level_id: com_preference_activity_levels(:info).id,
    )

    assert_not audit.valid?
    assert_includes audit.errors[:event_id], "must reference an existing com_preference_activity_event"
  end
  ...
  test "rejects unknown level_id before database foreign key enforcement" do
    audit = ComPreferenceActivity.new(
      ...
      occurred_at: Time.current,
      expires_at: 1.year.from_now,
      event_id: com_preference_activity_events(:create_new_preference_token).id,
      level_id: 999_999,
    )

    assert_not audit.valid?
    assert_includes audit.errors[:level_id], "must reference an existing com_preference_activity_level"
  end
