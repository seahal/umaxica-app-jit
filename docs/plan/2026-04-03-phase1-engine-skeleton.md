...
**GitHub Issue:** #553 **Parent design:** `docs/engine.md` **Goal:** Create a minimal, loadable
Rails mountable engine at `engines/local/` and the `Jit::Deployment` gate module. No controllers,
models, or routes are moved in this phase. The engine loads, the app boots, and all existing tests
pass.
...
## Step 8: Update `docs/engine.md` DB Classification

The DB classification in `docs/engine.md` (lines 32-48) has two errors. Apply these corrections:
...
| `docs/engine.md`   | Fix DB classification (guest/finder → Global)                |
