...
- `README.md`, `docs/tasks/checklist.md`
...
- Keep authentication models separate by responsibility. For BFF-based web flows, prioritize CSRF
  protection and operability with cookie sessions; for native clients such as iOS, use Bearer (JWT).
  Both approaches may coexist, but do not manage them twice for the same client.
...
- Sequence diagrams and state flows live in `docs/reference/uml/`.
- Environment variable catalog referenced in `.env.example` (future addition).
- Testing strategy captured in `docs/tasks/test.md`.
