...
このアプリケーションは、**Pundit**ベースの認可システムを実装しています。
**RBAC（Role-Based Access Control）**と**リソースベース認可**を組み合わせたハイブリッドアプローチを採用しています。
...
# いずれかのロールを持つか
user.has_any_role?('admin', 'manager', organization: organization)
...
```ruby
# デバッグ用コード
user.roles.pluck(:key)  # => ["admin", "editor"]
user.has_role?('admin', organization: org)  # => true/false
```
