phase 1 のようけんていぎしょですが、まーじしてください。> 認証・認可システム 要件定義書（Phase 1）
1. 目的・背景

本システムは、ユーザーに対して安全かつ柔軟な認証・認可機能を提供し、
同時にユーザー自身が セキュリティ状態・ログイン履歴・退会状態を把握・管理できることを目的とする。

特に以下を重視する。

複数の認証手段を組み合わせた高い安全性

ロックアウトを防ぐ設計

ユーザー主導のセキュリティ管理

プライバシーおよび GDPR 等への配慮

2. 対象範囲
対象

ユーザー向け認証・認可・設定機能

サインアップ / サインイン

セッション管理

ログアウト

認証手段の追加・削除

アクティビティ確認

退会・復帰・匿名化

非対象

スタッフによるユーザー選択型の退会処理

管理者向け監査・SIEM 連携

法務要件の詳細定義（保持年限など）

3. 認証手段（Authentication Methods）
利用可能な認証手段

Email（OTP）

Telephone（SMS OTP）

Passkey

Secret

Google ソーシャルログイン

Apple ソーシャルログイン

認証手段の基本方針

Email / Telephone / Secret は 値の更新（上書き）を行わない

変更は「削除＋新規追加」で行う

Passkey は例外として 表示名のみ変更可能

Social Login（Google / Apple）は 連携・解除のみ

4. サインイン要件
Passkey サインイン

Passkey によるサインインは、以下 3要素すべてが必須とする。

PII（Email または Telephone）

Passkey による認証

Cloudflare Turnstile（stealth / 非表示）

Google / Apple サインイン

Turnstile は使用しない

MFA 要求状態であっても、追加チャレンジへは遷移しない

5. セッション管理要件
同時セッション数

モデル上の最大セッション数：3

同時に通常利用可能なセッション数：2

セッション状態

3つ目のセッションは 隔離状態

ログインは成立しているが、ログイン必須ページにはアクセス不可

4つ目のセッション作成は ログイン時点で拒否

セッション管理画面

アクティブなセッションのみ表示

現在のセッションは削除不可

他セッションのみ失効可能（Refresh Token 無効化）

6. ログアウト要件

ログアウト操作により、当該セッションの Refresh Token を失効する

Access Token は即時失効できないため、以後のアクセスは状態チェックで拒否する

7. 認証手段の削除・解除ルール
Social Login（Google / Apple）

解除後もログイン可能な手段が 1つ以上存在する場合のみ解除可能

Passkey

全削除する場合、以下のいずれかが存在すること

Email

Google

Apple

Secret は条件に含めない

Secret

原則削除可能

削除後に Secret のみでログイン可能な状態を作らない

Email / Telephone（連絡手段）

Email と Telephone は 連絡手段として扱う

操作の結果として
Email + Telephone の合計が「1以上 → 0」になる遷移を禁止

Email 全削除条件

Telephone が1件以上存在 かつ

Passkey / Google / Apple のいずれかが存在

Telephone 全削除条件

Email が1件以上存在

8. アクティビティ表示（/configuration/activity）
目的

ユーザーが自身のログイン履歴・操作履歴を確認できること

不審なログインを早期に発見できること

表示内容（最低限）

日時

イベント種別（ログイン / ログアウト / セッション失効 等）

ログイン手段

端末・ブラウザ概要

IP（部分マスク）

9. 退会（Withdraw）要件
基本方針

ユーザー自身のみが実行可能

即時停止（利用不可）とする

復帰可能期間は31日（必須）

退会後の状態

退会後31日間は復帰可能

31日経過後は復帰不可

パージ（匿名化）

物理削除は行わない

個人情報を匿名化する 論理削除

バッチ処理により実行

実行タイミングは 31日経過後、概ね32日以内

強制匿名化

例外的に31日を待たずに実行可能

バッチ処理による実行

UI 提供は Phase 1 では行わない

10. 非機能要件（抜粋）

セキュリティ

ロックアウト防止

高リスク操作は ReAuth 必須

プライバシー

最小限のログ保持

GDPR 等を考慮した匿名化

可用性

複数セッション利用を許可

11. 未決事項（Phase 2 以降）

復帰不能フラグの具体実装

アクティビティの自動異常検知

匿名化の詳細粒度

ログ保持期間
