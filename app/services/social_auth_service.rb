...
class SocialAuthService
  ...
  def handle_callback
    ...
    PrincipalRecord.transaction do
      case @intent
      when "signup"
        handle_login(identity_class, provider, uid, allow_new_user: true)
      when "link"
  ...
  private
    ...
    def handle_login(identity_class, provider, uid, allow_new_user: false)
      identity = identity_class.lock.find_by(uid: uid, provider: provider)

      if identity&.active? && identity.user&.active?
        user = identity.user
        identity.update_from_auth_hash!(@auth_hash)
        return build_result(user, identity, reauthenticated: false, existing_account: true)
      end

      unless allow_new_user
        raise SocialAuth::ConflictError.new(
          "errors.social_auth.identity_not_found",
      ...
      if identity
        ...
        user, identity = build_user_and_identity(identity_class, provider, uid)
      end
    ...
    def build_user_and_identity(identity_class, provider, uid)
      user = User.new(status_id: UserStatus::UNVERIFIED_WITH_SIGN_UP)
