import { createRoot, type Root } from "react-dom/client";
import HelpComLanding from "../../components/help/com/Landing";

const ROOT_ID = "help-com-react-root";

let root: Root | null = null;

const mount = () => {
	const container = document.getElementById(ROOT_ID);

	if (!container) {
		return;
	}

	// Avoid remounting if Turbo caches the element and our root already exists.
	if (!root) {
		root = createRoot(container);
	}

	root.render(<HelpComLanding />);
};

const unmount = () => {
	if (root) {
		root.unmount();
		root = null;
	}
};

const registerTurboEvents = () => {
	document.addEventListener("turbo:load", mount);
	document.addEventListener("turbo:before-render", unmount);
};

const unregisterTurboEvents = () => {
	document.removeEventListener("turbo:load", mount);
	document.removeEventListener("turbo:before-render", unmount);
};

if (document.readyState === "loading") {
	document.addEventListener("DOMContentLoaded", mount);
	document.addEventListener("DOMContentLoaded", registerTurboEvents, { once: true });
} else {
	mount();
	registerTurboEvents();
}

// Guard against module hot-reloads (Bun watch) by cleaning up listeners.
if (import.meta && import.meta.hot) {
	import.meta.hot.dispose(() => {
		unregisterTurboEvents();
		unmount();
	});
}
