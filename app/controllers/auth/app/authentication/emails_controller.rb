      ...
      class EmailsController < ApplicationController
        ...
        def create
          ...
          unless normalized_address
            @user_email = UserIdentityEmail.new(address: address)
            @user_email.errors.add(:address, t("auth.app.authentication.email.create.invalid_format"))
            return render :new, status: :unprocessable_content
          end
          ...
          redirect_params = { notice: t("auth.app.authentication.email.create.verification_code_sent") }
          redirect_params[:rd] = params[:rd] if params[:rd].present?
          session[:user_email_authentication_rd] = params[:rd] if params[:rd].present?

          redirect_to edit_auth_app_authentication_email_path(redirect_params)
        end
          ...
          if result[:success]
            respond_to do |format|
              format.html do
                ...
                if rd_param.present?
                  flash[:notice] = t("auth.app.authentication.email.update.success")
                  jump_to_generated_url(rd_param)
                else
                  redirect_to "/", notice: t("auth.app.authentication.email.update.success")
                end
              end
              format.json do
        ...
        def handle_failed_otp_attempt(user_email, user = nil)
          ...
          if user_email.locked?
            ...
            { success: false, error: t("auth.app.authentication.email.update.invalid_code", attempts_left: remaining) }
          end
        end
