      ...
      class EmailsController < ApplicationController
        ...
        def create
          ...
          unless turnstile_result["success"]
            @user_email.errors.add(:base, t("auth.app.registration.email.create.turnstile_validation_failed"))
          ...
          redirect_to edit_auth_app_registration_email_path(@user_email.id, redirect_params)
        end
        ...
        def update
          ...
          if otp_data.blank? || submitted_code.blank?
            @user_email.errors.add(:pass_code, t("auth.app.registration.email.update.invalid_code"))
          ...
          unless ActiveSupport::SecurityUtils.secure_compare(expected_code, submitted_code)
            ...
            if @user_email.locked?
              @user_email.destroy!
              redirect_params = { alert: t("auth.app.registration.email.update.attempts_exceeded") }
              redirect_params[:rd] = params[:rd] if params[:rd].present?
              redirect_to new_auth_app_registration_email_path(redirect_params) and return
            end
            @user_email.errors.add(:pass_code, t("auth.app.registration.email.update.invalid_code"))
          ...
          begin
            ...
            @user_email.errors.add(:base, t("auth.app.registration.email.update.failed"))
            render :edit, status: :unprocessable_content and return
          end
          ...
          if params[:rd].present?
            flash[:notice] = t("auth.app.registration.email.update.success")
            jump_to_generated_url(params[:rd])
          else
            redirect_to "/", notice: t("auth.app.registration.email.update.success")
          end
        end
