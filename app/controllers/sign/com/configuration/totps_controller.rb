  ...
  module Com
    module Configuration
      class TotpsController < ApplicationController
        ...
        def index
          @totps = current_customer.user_one_time_passwords
        end

        def new
          if current_customer.user_one_time_passwords.count >= MAX_TOTPS
        ...
        def create
          ...
          if @totp.private_key.blank?
            redirect_to(
              new_sign_com_configuration_totp_path,
        ...
        def update
          ...
          if @totp.update(update_params)
            redirect_to(
              sign_com_configuration_totps_path(ri: params[:ri]),
        ...
        def find_totp
          current_customer.user_one_time_passwords.find_by!(public_id: params[:id])
        end
        ...
        def account_id
          current_customer.user_emails.first&.address || current_customer.public_id
        end
