    ...
    module Configuration
      class PasskeysController < ApplicationController
        ...
        def verify
          ...
          render json: { status: "ok", redirect_url: sign_app_configuration_passkeys_path }
        rescue WebAuthn::Error => e
          render json: { error: e.message }, status: :unprocessable_content
            ...
            if @passkey.save
              format.html {
                redirect_to sign_app_configuration_passkey_path(@passkey),
                            notice: t("messages.passkey_successfully_created")
              }
              format.json { render :show, status: :created, location: sign_app_configuration_passkey_path(@passkey) }
            else
        ...
        def update
          ...
          respond_to do |format|
            if @passkey.update(passkey_params)
              ...
              format.json { render :show, status: :ok, location: sign_app_configuration_passkey_path(@passkey) }
            else
        ...
        def destroy
          ...
          respond_to do |format|
            format.html {
              redirect_to sign_app_configuration_passkeys_path, status: :see_other,
