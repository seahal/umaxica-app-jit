        ...
        private
          ...
          def handle_successful_auth(user, intent, provider_name, identity, existing_account: nil)
            ...
            when "reauth"
              ...
              rd_param = peek_redirect_parameter
              sign_in(user)
              session[::Auth::Base::DEFAULT_RD_SESSION_KEY] = rd_param if rd_param.present?
              if existing_account
                redirect_with_notice(
                  social_auth_success_redirect_path,
                  I18n.t("sign.app.social.sessions.create.already_registered", provider: provider_name),
                )
              else
                redirect_with_notice(
                  social_auth_success_redirect_path,
                  I18n.t("sign.app.social.sessions.create.success", provider: provider_name),
                )
              end
            end
          end
