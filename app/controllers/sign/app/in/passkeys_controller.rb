  ...
  module App
    module In
      ...
      class PasskeysController < ApplicationController
        ...
        def options
          identifier = (params[:identifier].presence || params[:email]).to_s.strip
        ...
        private
          ...
          def find_active_user_by_identifier(identifier)
            clean_email = identifier.downcase
            user_email = UserEmail.find_by(address: clean_email, user_identity_email_status_id: UserEmailStatus::VERIFIED)
            user = user_email&.user
            return user if user&.active?

            telephone = find_verified_telephone(identifier)
            user = telephone&.user
            return user if user&.active?
          ...
          def find_verified_telephone(value)
            normalized_number = normalize_telephone_identifier(value)
            return nil if normalized_number.blank?

            UserTelephone.find_by(number: normalized_number, user_identity_telephone_status_id: UserTelephoneStatus::VERIFIED)
          end

          def normalize_telephone_identifier(value)
          ...
          def handle_login_result(result)
            ...
            when :session_limit_exceeded
              ...
              render json: {
                ...
                redirect_url: edit_sign_app_in_session_path
              }, status: :ok
          ...
          def render_success(result)
            redirect_url = retrieve_redirect_url || sign_app_configuration_path
            render json: {
              status: "ok",
